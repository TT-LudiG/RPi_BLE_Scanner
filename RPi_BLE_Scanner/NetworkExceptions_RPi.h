...
namespace NetworkExceptions_RPi
{
    // EXCEPTION_NET_RPI_0
    ...
    public:
        ServerConnectException(const std::string servername, const std::string port, const std::string error): _servername(servername), _port(port), _error(error) {}
	
        virtual const char* what() const throw()
        {
            std::string message = "EXCEPTION_NET_RPI_2: Failed to connect to the server: " + _servername + ", at port: " + _port + ", with error: " + _error;
    ...
    public:
        ...
        virtual const char* what() const throw()
        {
            std::string message = "EXCEPTION_NET_RPI_3: Failed to write to the socket: " + std::to_string(_socketHandle) + ", with error: " + _error;
    ...
    public:
        ...
        virtual const char* what() const throw()
        {
            std::string message = "EXCEPTION_NET_RPI_4: Failed to read from the socket: " + std::to_string(_socketHandle) + ", with error: " + _error;
